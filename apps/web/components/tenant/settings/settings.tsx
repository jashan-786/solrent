...
export function SecurityPrivacy({ lease }: { lease: any }) {
    ...
    const handleViewOnChain = () => {
        if (!lease?.leaseNftMint) {
            alert("No on-chain record found.");
            return;
        }
        window.open(`https://solscan.io/token/${lease.leaseNftMint}?cluster=devnet`, "_blank");
    };
        ...
        <section className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div 
                    ...
                    className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex items-center justify-between group hover:border-sol-indigo transition-colors cursor-pointer"
                >
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-text-grey uppercase font-bold tracking-tighter">
                                {lease?.startDate ? `Signed ${new Date(lease.startDate).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}` : 'Not Signed'} • PDF
                            </p>
                        </div>
                    </div>
                ...
                >
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-text-grey uppercase font-bold tracking-tighter">
                                {lease?.leaseNftMint ? 'On-Chain Certificate' : 'Pending Verification'}
                            </p>
                        </div>
                    </div>
