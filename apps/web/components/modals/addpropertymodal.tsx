...
export function AddPropertyModal() {
    ...
    const handleSubmit = async () => {
        setLoading(true);
        try {
            await axios.post("/api/landlord/buildings", {
                name,
                address,
                city,
                province: state,
            ...
            <DialogContent className="sm:max-w-[600px] bg-white border-none shadow-2xl max-h-[90vh] overflow-y-auto">
                ...
                <div className="grid gap-6 py-6">
                    <div className="grid gap-2">
                        ...
                        <Input id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Skyline Loft" className="bg-background-100 border-none" />
                    </div>

                    <div className="grid gap-2">
                        <Label htmlFor="address" className="text-tiny font-bold uppercase text-primary-900">Street Address</Label>
                        <Input id="address" value={address} onChange={(e) => setAddress(e.target.value)} placeholder="123 Main St" className="bg-background-100 border-none" />
                    </div>

                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                        <div className="grid gap-2">
                            <Label htmlFor="city" className="text-tiny font-bold uppercase text-primary-900">City</Label>
                            <Input id="city" value={city} onChange={(e) => setCity(e.target.value)} placeholder="Winnipeg" className="bg-background-100 border-none" />
                        </div>
                        <div className="grid gap-2">
                            <Label htmlFor="state" className="text-tiny font-bold uppercase text-primary-900">Province</Label>
                            <Input id="state" value={state} onChange={(e) => setState(e.target.value)} placeholder="MB" className="bg-background-100 border-none" />
                        </div>
                        <div className="grid gap-2 col-span-2 md:col-span-1">
                            ...
                            <Input id="zip" value={zip} onChange={(e) => setZip(e.target.value)} placeholder="R3C 2P4" className="bg-background-100 border-none" />
                        </div>
                    </div>
                </div>
